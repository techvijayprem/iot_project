...
const unsigned long disconnectDuration = 3000; // 10 minutes in milliseconds  5
const unsigned long reconnectDuration = 1200; // 1 hour in milliseconds     2
...
void serverconfig() {
 ...
 // WiFi.disconnect(true); // true parameter disconnects all clients

  Serial.println("All connected devices disconnected from ESP8266 WiFi extender.");
      delay(100);
      //ESP.reset();
    }
  ...
  if ((currentTime - lastActivityTime) > disconnectDuration) {
    handleDisconnect();
  }
}
...
void setup() {
  ...
  serverconfig();
    ...
    timer.attach(60, checkInactivity); // Attach the timer to the checkInactivity function with a 60-second interval
...
void loop() {
  ...
  // Handle reconnection restriction
  unsigned long currentTime = millis();
  if ((currentTime - lastActivityTime) > reconnectDuration) {
    // Allow reconnection after the specified duration
    WiFi.mode(WIFI_AP_STA);
  }

  // Run the timer for checking inactivity
    server.handleClient();
}
